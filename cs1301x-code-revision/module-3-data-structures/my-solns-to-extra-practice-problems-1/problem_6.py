...
def check_formula(input_string):
  ...
  try:
    ...
    if input_string.find("=") == len(input_string)-1:
      return False
    
    for operator in operators:
      if input_string.find(operator) == 0:
        return False
    
    for i in range(len(input_string)-1):         
      if input_string[i] in operators and input_string[i+1] in operators:
        return False
      if input_string[i] not in operators and not input_string[i].isdigit():        
        return False
